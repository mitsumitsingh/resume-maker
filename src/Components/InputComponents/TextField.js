...
function TextField(props) {
  ...
  const checkValidation = () => {
    ...
    if (props.validation && props.validation.required && value === "") {
      ...
      errorMessage =
        "write upto " + props.validation.maxLengthRequired + " characters";
    } else if (
      props.validation &&
      props.validation.checkType &&
      props.validation.checkType === "email"
    ) {
  ...
  return (
    <div className="w-100 h-100 position-relative">
      ...
      <div
        style={
          (value !== "" || showErrorMessages === true) && errorMessage !== ""
            ? {
                display: "block",
                position: "absolute",
                bottom: -20,
